...
from .forms import UstaForm, CreateUserForm, UserUpdateForm, ProfileUpdateForm
...
def profile_update(request):
    if request.method == 'POST':
        ...
        usta_form = ProfileUpdateForm(request.POST, request.FILES, instance=request.user.usta)
        if user_form.is_valid() and usta_form.is_valid():
            user_form.save()
            usta_form.save()
            return redirect('user-profile')
    else:
        user_form = UserUpdateForm(instance=request.user)
        usta_form = ProfileUpdateForm(instance=request.user.usta)
